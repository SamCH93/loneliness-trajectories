...
int_plot_fun <- function(d, int, mod, cov, random, imp){
  ...
  if(int == "b.age"){
    d <- d %>%
      filter((b.age == 15 & age > 0) | (b.age == -15 & age < 1))
  }
  p <- d %>% 
...
fp_group_plot_fun <- function(d, imp, mod, rand, int, cov){
  ...
  ttl <- if(int == "Interaction") sprintf("%s Change in Loneliness", mod) else "Loneliness Levels"
...
fplot_int_panels <- fplot_nested %>%
  ...
  mutate(group = mapvalues(term, mod_vals$old, mod_vals$group)) %>%
