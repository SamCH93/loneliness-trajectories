...
- Need to finalize LISS, BHPS, OCTO, SHP  
- How to code b.age for interaction plots  
- How to organize all the various terms in tables  
- Formatting of the forest plots  
...
# N @basline                            X
# Number of MO’s                        X
# Intervals between MO (mean or mode)   X
# Baseline year                         X 
# Country         
# Baseline age (mean, range)            X
# %Female                               X
# Loneliness mean/sd (baseline)         X
# Loneliness scale (UCLA or CES-D)
# Isolation mean/sd (baseline)          X
# % married/divorced/widow/single       X
# Education mean/sd                     X
# Functional limitations (mean/sd)      X
...
man_desc_tab <- man_desc_tab %>%
  ...
  kable_styling(full_width = F); man_desc_tab
...
nested_res <- tibble(
  ...
  separate(file, c("Imputed", "study", "model", "random", "int", "cov"), sep = "_") %>% 
  filter(!is.na(data)) %>%
  filter(study != "BHPS") %>%
  filter(!(study == "OCTOTWIN" & model == "Quadratic" & int == "Interaction")) %>%
  # filter(int == "b.age") %>%
  unnest(data)
```
...
stdy_tab_fun <- function(imp, mod, rand, int, cov, d){
  ...
  cap <- paste(cap, if(int == "Interaction") "with Predictor Main Effects and Interactions" else "with Predictor Main Effects") 
...
# What is linear.main/quad.main? I don't remember it from the original script but it appears also need meta-analyses of these covariate point estimates? Are these separate models or one big one? 
...
nested_meta_tab <- nested_meta %>%
  ...
  select(Imputed, model, random, int, cov, term, estimate, std.error, p) %>%
  group_by(Imputed, model, random, int, cov) %>%
  nest() %>%
  ungroup() %>%
...
mod_vals <- tribble(
  ...
  , "b.age-15"    , "Low Baseline Age"           , "b.age"
  , "b.age15"     , "High Baseline Age"          , "b.age"
)
...
int_plot_fun <- function(d, int, mod, cov, random, imp){
  ...
  if(int == "b.age"){
    d <- d %>%
      filter((b.age == 15 & age > 0) | b.age == -15)
  }
  p <- d %>% 
...
int_traj_plots <- crossing(
  ...
  mutate(p = pmap(list(data, term, model, cov, random, Imputed), int_plot_fun))
```

### Forest Plots  

Add N & heterogeneity estiamtes. Add titles
```{r}
f_plot_fun <- function(d, trm, imp, mod, random, int, cov, I2, Q, p, df){
  ...
  stds <- d$study[!d$study %in% c("meta", " ")]
  d <- d %>%
    mutate(study = factor(study, rev(c(" ", stds, "meta")))
  ...
  p1 <- d %>%
    mutate(meta = ifelse(study == "meta", "meta", "study")) %>%
  ...
  p2 <- d2 %>%
    # mutate_at(vars(estimate, lower, upper)
    #           , ~ifelse(abs(.) < .01, sprintf("%.3f", .), sprintf("%.2f", .))) %>%
    # mutate(est = sprintf("%s [%s, %s]", estimate, lower, upper)) %>%
    ggplot(aes(x = study, y = est)) +
      geom_text(aes(label = value), hjust = .5, size = 3.5) + 
    
      # geom_text(aes(label = n.group), hjust = 1) +
      annotate("text", label = "b [CI]", x = length(stds) + 1.75, y = "est", hjust = .5, vjust = 0) +
      annotate("text", label = "N", x = length(stds) + 1.75, y = "n.group", hjust = .5, vjust = 0) +
      # geom_text(aes(label = "N", x = length(stds) + 2), hjust = 1) +
      geom_vline(aes(xintercept = 1.5)) +
      geom_vline(aes(xintercept = length(stds) + 1.5)) +
      coord_flip() +
      theme_void() +
...
fp_group_plot_fun <- function(d, imp, mod, rand, int, cov){
  ...
  ggsave(file = sprintf("%s/03-results/09-plots/03-forest-plot-panels/%s-%s-%s-%s-%s.png"
                        , wd, imp, mod, rand, int, cov)
         , width = 11.5
         , height = 14)
  ggsave(file = sprintf("%s/03-results/09-plots/03-forest-plot-panels/%s-%s-%s-%s-%s.pdf"
                        , wd, imp, mod, rand, int, cov)
         , width = 11.5
         , height = 14)
}
